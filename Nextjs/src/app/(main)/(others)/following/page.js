...
import { faSpinner, faExclamationTriangle, faUserPlus } from '@fortawesome/free-solid-svg-icons';
...
export default function FollowingPage() {
  ...
  const fetchPosts = useCallback(async (page = 1, append = false) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_FLASK_API_URL}/api/posts/following/${session.user.id}?page=${page}`);
      if (!response.ok) {
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#111] text-white p-4">
        ...
        >
          ...
          <p className="text-gray-500 text-sm mt-1">Récupération des posts de vos abonnements.</p>
        </motion.div>
      </div>
    );
  }
    ...
    return (
      <div className="flex justify-center items-center min-h-screen bg-[#111]">
        <div className="text-center">
          ...
          <p className="text-white text-xl mb-4">Vous devez être connecté pour voir cette page</p>
          <Link href="/login">
            <button className="bg-[#90EE90] text-black px-6 py-2 rounded-full hover:bg-[#7CD37C] transition-colors">
              Se connecter
            </button>
          </Link>
        </div>
    ...
    <div className="min-h-screen bg-[#111] text-white">
      <div className="max-w-6xl mx-auto px-4 py-8">
        <motion.div
          ...
          className="mb-8"
        >
          <h1 className="text-3xl font-bold text-[#90EE90] mb-2">Abonnements</h1>
          <p className="text-gray-400">Posts des personnes que vous suivez</p>
        </motion.div>
        ...
        {posts.length === 0 ? (
          ...
          >
            ...
            <p className="text-gray-500 mb-6">
              Découvrez et suivez des personnes pour voir leurs publications ici.
            </p>
            <Link href="/foryou">
              <button className="bg-[#90EE90] text-black px-6 py-2 rounded-full hover:bg-[#7CD37C] transition-colors">
        ...
        {!hasNext && posts.length > 0 && (
          <div className="text-center py-8">
            <p className="text-gray-400">Vous avez vu tous les posts de vos abonnements !</p>
          </div>
        )}
      </div>
    </div>
