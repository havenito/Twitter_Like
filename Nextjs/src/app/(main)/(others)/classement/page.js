...
export default function ClassementPage() {
  ...
  useEffect(() => {
    async function fetchClassement() {
      setLoading(true);
      try {
        const res = await fetch("http://localhost:5000/api/classement/top10");
        const data = await res.json();
        setTopUsers(data.top10 || []);

        if (session?.user?.id) {
          const resRank = await fetch(`http://localhost:5000/api/classement/user/${session.user.id}`);
  ...
  return (
    <div className="min-h-screen bg-[#111] text-white flex flex-col items-center">
      <main className="w-full max-w-2xl px-2 sm:px-4 py-8 sm:py-12">
        ...
        <section className="bg-[#181c24] rounded-2xl shadow-xl p-4 sm:p-8 mb-10 w-full">
          ...
          <div className="flex justify-center items-end gap-2 sm:gap-8 mb-12 w-full relative">
              ...
              <div className="flex flex-col items-center w-20 sm:w-28 z-10 relative">
                ...
                >
                  <span className="absolute -top-6 left-1/2 -translate-x-1/2 text-xl font-bold"
                        style={{ color: "#23272f", textShadow: "0 1px 4px #fff8" }}>2</span>
                </div>
                <span
            ...
            {podium[0] && (
              <div className="flex flex-col items-center w-24 sm:w-36 z-20 relative">
                ...
                >
                  <span className="absolute -top-6 left-1/2 -translate-x-1/2 text-xl font-bold"
                        style={{ color: "#23272f", textShadow: "0 1px 4px #fff8" }}>1</span>
                </div>
                <span
            ...
            {podium[2] && (
              <div className="flex flex-col items-center w-16 sm:w-24 z-10 relative">
                ...
                >
                  <span className="absolute -top-6 left-1/2 -translate-x-1/2 text-xl font-bold"
                        style={{ color: "#23272f", textShadow: "0 1px 4px #fff8" }}>3</span>
                </div>
                <span
