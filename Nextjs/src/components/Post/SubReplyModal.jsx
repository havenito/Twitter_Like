...
const SubReplyModal = ({ isOpen, onClose, parentReply, onSubReplyAdded }) => {
  ...
  const handleSubmitSubReply = async (e) => {
    e.preventDefault();
    ...
    try {
      ...
      formData.append('replies_id', parentReply.id);
        ...
        {(isOpen || isClosing) && (
          ...
          >
            <motion.div
              ...
              transition={{ duration: 0.3 }}
              className="bg-[#1b1b1b] rounded-xl max-w-lg w-full max-h-[80vh] overflow-hidden border border-[#333]"
            ...
            >
              ...
              {parentReply && (
                <div className="p-4 border-b border-[#333] bg-[#1e1e1e]">
                  <div className="flex items-start space-x-3">
                    {parentReply.user?.pseudo ? (
                      <Link href={`/${parentReply.user.pseudo}`}>
                        {renderProfilePicture(parentReply.user, 'w-10 h-10')}
                      </Link>
                    ) : (
                      renderProfilePicture(parentReply.user, 'w-10 h-10')
                    )}
                    <div className="flex-1">
                      ...
                      <p className="text-gray-300 text-sm line-clamp-3">{parentReply.content}</p>
                    </div>
                  </div>
                </div>
              )}
              ...
              <div className="p-6 max-h-96 overflow-y-auto">
                <form onSubmit={handleSubmitSubReply} className="space-y-4">
                  <div className="flex items-start space-x-3">
                      ...
                      <div className="flex justify-between items-center mt-3">
                        <div className="flex items-center space-x-3">
                          ...
                          <span className="text-xs text-gray-500">
                            {replyContent.length}/500 • {mediaFiles.length}/4 médias
                          </span>
                        </div>
                        <div className="flex space-x-3">
                          ...
                          <button
                            type="submit"
                            disabled={submitting || (!replyContent.trim() && mediaFiles.length === 0) || isClosing}
