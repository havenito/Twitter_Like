...
const ReplyModal = ({ isOpen, onClose, comment, onReplyAdded, isReplyToReply = false }) => {
  ...
  const handleSubmitReply = async (e) => {
    ...
    try {
      ...
      if (isReplyToReply) {
        formData.append('replies_id', comment.id);
      } else {
            ...
            >
              <div className="flex items-center justify-between p-4 border-b border-[#333]">
                <h2 className="text-xl font-bold text-white flex items-center">
                  ...
                  {isReplyToReply ? 'Répondre à la réponse' : 'Répondre au commentaire'}
                </h2>
                <button
                  onClick={handleClose}
                  disabled={submitting}
              ...
              {comment && (
                <div className="p-4 border-b border-[#333] bg-[#1e1e1e]">
                  <div className="flex items-start space-x-3">
                    {comment.user?.pseudo ? (
                      <Link href={`/${comment.user.pseudo}`}>
                        {renderProfilePicture(comment.user, 'w-10 h-10')}
                      </Link>
                    ) : (
                      renderProfilePicture(comment.user, 'w-10 h-10')
                    )}
