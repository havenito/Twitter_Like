...
const CommentForm = ({ postId, onCommentAdded }) => {
  ...
  const centeredNotificationVariants = {
    initial: { opacity: 0, x: -175, y: -50, scale: 0.3 },
    animate: { opacity: 1, x: -175, y: 0, scale: 1 },
    exit: { opacity: 0, x: -175, y: -20, scale: 0.5, transition: { duration: 0.4 } }
  };
              ...
              <div className="flex justify-between items-center mt-3">
                <div className="flex items-center space-x-3">
                  ...
                  <span className="text-xs text-gray-500">
                    {comment.length}/500 • {mediaFiles.length}/4 médias
                  </span>
                </div>
                
                <button
                  type="submit"
                  disabled={submitting || (!comment.trim() && mediaFiles.length === 0)}
